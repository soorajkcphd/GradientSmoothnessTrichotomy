...
def exp5_convergence_rate(d=30, T_values=None, n_seeds=N_SEEDS):
    ...
    stds = np.array([np.std(results[T]) for T in T_values])
    log_m_lo = np.log(np.maximum(means - stds, 1e-15))
    log_m_hi = np.log(means + stds)

    plt.figure(figsize=(5, 4))
    plt.plot(log_T, log_m, 'o-', color='tab:blue', ms=8, label='Mean (5 seeds)')
    plt.fill_between(log_T, log_m_lo, log_m_hi, alpha=0.20, color='tab:blue',
                     label='Mean +/- 1 std')
    plt.plot(log_T, log_m[0] - 0.5 * (log_T - log_T[0]), '--', color='gray',
             label='O(T^{-1/2})')
    plt.xlabel("log T")
...
def exp5b_stochastic_convergence_rate(d=30, T_values=None, n_seeds=N_SEEDS, eta=0.25, sigma_g=1.0):
    ...
    stds_b = np.array([np.std(results[T]) for T in T_values])
    log_m_lo_b = np.log(np.maximum(means - stds_b, 1e-15))
    log_m_hi_b = np.log(means + stds_b)

    plt.figure(figsize=(5, 4))
    plt.plot(log_T, log_m, 'o-', color='tab:blue', ms=8, label='Mean (5 seeds)')
    plt.fill_between(log_T, log_m_lo_b, log_m_hi_b, alpha=0.20, color='tab:blue',
                     label='Mean +/- 1 std')
    plt.plot(log_T, log_m[0] - 0.5 * (log_T - log_T[0]), '--', color='gray',
             label='O(T^{-1/2})')
    plt.xlabel("log T")
    plt.ylabel("log E[||grad J||^2]")
    plt.title(f"Stochastic Convergence Rate (slope = {slope:.2f})")
    plt.legend()
...
def exp11_empirical_lipschitz(R_values=None, n_pairs=300, n_seeds=5):
    """
    Experiment 11: Empirical Lipschitz constant L_hat(R) vs search radius R.

    Directly validates the representation-optimization trichotomy:
      SO(3)  -> L_hat(R) ~ O(1)          (flat on log-log)
      GL(3)  -> L_hat(R) ~ O(exp(2R))    (linear on log-log vs R)
      SE(3)  -> L_hat(R) ~ O(R^2)        (slope ~2 on log-log)

    For each algebra and each radius R, we sample n_pairs random pairs
    (theta, theta') uniformly on the Frobenius sphere of radius R,
    compute numerical gradients of J(theta) = (1/2)||exp(theta) - A||_F^2,
    and estimate L_hat(R) = max ||grad J(theta) - grad J(theta')|| / ||theta - theta'||.
    """
    if R_values is None:
        R_values = [0.5, 1.0, 2.0, 4.0, 8.0]

    print("\n" + "=" * 70)
    print("EXPERIMENT 11: Empirical Lipschitz constant L_hat(R) vs R")
    print("  Validates trichotomy: SO(3)=O(1), GL(3)=O(exp(2R)), SE(3)=O(R^2)")
    print("=" * 70)

    eps_fd = 1e-5  # finite-difference step

    def numerical_grad(theta_vec, exp_fn, hat_fn, A, dim):
        """Numerical gradient of J(theta) = 0.5 ||exp(hat(theta)) - A||_F^2."""
        g = np.zeros(dim)
        for i in range(dim):
            ei = np.zeros(dim)
            ei[i] = 1.0
            Xp = exp_fn(hat_fn(theta_vec + eps_fd * ei))
            Xm = exp_fn(hat_fn(theta_vec - eps_fd * ei))
            g[i] = np.sum((0.5 * (Xp - A) ** 2 - 0.5 * (Xm - A) ** 2)) / eps_fd
        return g
    ...
    algebras = {
        'SO(3)': (so3_hat_vec, so3_exp_vec, 3, np.eye(3)),
        'GL(3)': (gl3_hat, gl3_exp, 9, np.eye(3)),
        'SE(3)': (se3_hat_vec, se3_exp_vec, 6, np.eye(4)),
    }

    colors = {'SO(3)': 'tab:blue', 'GL(3)': 'tab:red', 'SE(3)': 'tab:green'}

    all_results = {}

    for alg_name, (hat_fn, exp_fn, dim, A) in algebras.items():
        L_means = []
        L_stds = []
        print(f"\n  {alg_name} (dim={dim})")
        ...
        for R in R_values:
            L_per_seed = []
            for seed in range(n_seeds):
                rng = np.random.RandomState(seed * 100 + int(R * 10))
                L_max = 0.0
                for _ in range(n_pairs):
                    # Sample theta on Frobenius sphere of radius R
                    v = rng.randn(dim)
                    v = v / np.linalg.norm(v) * R * (0.5 + 0.5 * rng.rand())
                    vp = rng.randn(dim)
                    vp = vp / np.linalg.norm(vp) * R * (0.5 + 0.5 * rng.rand())
                    dist = np.linalg.norm(v - vp)
                    if dist < 1e-8:
                        continue
                    g  = numerical_grad(v,  exp_fn, hat_fn, A, dim)
                    gp = numerical_grad(vp, exp_fn, hat_fn, A, dim)
                    ratio = np.linalg.norm(g - gp) / dist
                    if ratio > L_max:
                        L_max = ratio
                L_per_seed.append(L_max)

            mu = np.mean(L_per_seed)
            sd = np.std(L_per_seed)
            L_means.append(mu)
            L_stds.append(sd)
            print(f"    R={R:.1f}: L_hat={mu:.4f} +/- {sd:.4f}")

        all_results[alg_name] = {'means': L_means, 'stds': L_stds}

    # ── Plot 1: L_hat(R) vs R on log-log axes ──────────────────
    plt.figure(figsize=(6, 4.5))
    R_arr = np.array(R_values, dtype=float)

    for alg_name, res in all_results.items():
        mu = np.array(res['means'])
        sd = np.array(res['stds'])
        c = colors[alg_name]
        plt.loglog(R_arr, mu, 'o-', color=c, label=alg_name, ms=7, lw=2)
        plt.fill_between(R_arr,
    ...
    R_ref = R_arr
    # O(1) reference
    plt.loglog(R_ref, np.ones_like(R_ref) * all_results['SO(3)']['means'][0],
               ':', color='tab:blue', alpha=0.5)
    # O(R^2) reference anchored at R=0.5
    C2 = all_results['SE(3)']['means'][0] / (R_values[0] ** 2 + 1e-9)
    plt.loglog(R_ref, C2 * R_ref ** 2, ':', color='tab:green', alpha=0.5,
               label='O(R^2)')
    # O(exp(2R)) reference anchored at R=0.5
    C_exp = all_results['GL(3)']['means'][0] / np.exp(2 * R_values[0])
    plt.loglog(R_ref, C_exp * np.exp(2 * R_ref), ':', color='tab:red', alpha=0.5,
               label='O(exp(2R))')

    plt.xlabel("Search radius $R$ (log scale)", fontsize=11)
    plt.ylabel("Empirical $\\hat{L}(R)$ (log scale)", fontsize=11)
    plt.title("Empirical Lipschitz constant vs radius\n"
              "(validates trichotomy: flat / exponential / quadratic)",
              fontsize=10)
    plt.legend(fontsize=9)
    plt.grid(True, which='both', alpha=0.3)
    save_fig("exp11_empirical_lipschitz.png")

    # ── Print fitted slopes ────────────────────────────────────
    print("\n  Fitted log-log slopes (theory: SO(3)~0, GL(3)~2, SE(3)~2):")
    for alg_name, res in all_results.items():
        mu = np.array(res['means'])
        log_R = np.log(R_arr)
        log_L = np.log(np.maximum(mu, 1e-12))
        slope, _ = np.polyfit(log_R, log_L, 1)
        print(f"    {alg_name}: slope = {slope:.3f}")
...
ALL_EXPERIMENTS = {
    ...
    11: ('Empirical Lipschitz L_hat(R) vs radius -- validates trichotomy', exp11_empirical_lipschitz),
}
...
def run_all(selected=None):
    ...
    print("\n" + "=" * 80)
    print("UNIFIED EXPERIMENTAL VALIDATION - CAM PAPER")
